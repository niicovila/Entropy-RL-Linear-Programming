...
from main.algorithms.classic.common_utils import Sampler, ExponentiatedGradientSampler, nll_loss, kl_loss
import logging
...
def tune_saddle(config):
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if args.save_learning_curve and len(mean_reward) > 0: 
            ...
            print(f'Iteration: {global_step}, Reward: {np.mean(mean_reward)}')
            mean_reward = []
        ...
        if len(reward_iteration) > 5: 
            ...
            full_rewards.append(np.mean(reward_iteration))
            reward_iteration = []
    ...
    if len(full_rewards) == 0: logging_callback(-2000)
